...
const TRANSCRIPT_TOOL_VERSION = "2.9.8-center-watermark-larger-deeper";
...
function collectIntegrityPayload() {
  ...
  return JSON.stringify({ docId, ack, editable, rows, personalSsn });
}
...
function syncTermSelectOptionsForActiveTerms() {
  ...
  const labels = Array.from(document.querySelectorAll(".term-block h2")).map((h2) => {
    ...
    return ((inner || h2).textContent || "").trim() || `Term`;
  });
  if (isSingleTermLayout()) {
    termSelect.innerHTML = "";
    const opt = document.createElement("option");
    opt.value = "0";
    opt.textContent = `Term (${labels[0] || "Term 1"})`;
  ...
  termSelect.options[0].textContent = `Term 1 (${labels[0] || "?"})`;
  termSelect.options[1].textContent = `Term 2 (${labels[1] || "?"})`;
}
...
async function generateExportQRPayload() {
  ...
  const payload = {
    ...
    msg: "Local tamper-evidence tag for this transcript session; not an official certification gateway.",
  };
...
async function refreshLiveExportQrPreview() {
  ...
  try {
    ...
    const previewPayload = {
      ...
      out,
      audit,
      msg: "Preview label; exporting PNG replaces this with the full export binding (includes raw snapshot).",
    };
...
function showUnsupportedImageAlert(file) {
  ...
  setUploadDiagnostics(
    `Upload status: Failed - unsupported image type (${filename}). Please use PNG/JPG/JPEG/WEBP/BMP/GIF/SVG/TIFF.`,
    "error"
  );
  window.alert(
    `Unsupported or unreadable image file: ${filename}\nPlease upload PNG/JPG/JPEG/WEBP/BMP/GIF/SVG/TIFF (HEIC/HEIF depends on browser support).`
  );
}
...
function bindProtectionControls() {
  ...
  const applyAckStatus = () => {
    if (ack.checked) {
      ackStatus.textContent = "Authorization status: Acknowledged";
      ackStatus.style.color = "#0a5d16";
    } else {
      ackStatus.textContent = "Authorization status: Not acknowledged";
...
function registerAutoEditableCells() {
  ...
  termBlocks.forEach((block, termIdx) => {
    const termTitle = block.querySelector("h2")?.textContent?.trim() || `Term ${termIdx + 1}`;
    const headers = Array.from(block.querySelectorAll("thead th")).map((th) => th.textContent.trim());
    ...
    Array.from(block.querySelectorAll("tbody tr")).forEach((row, rowIdx) => {
      Array.from(row.querySelectorAll("td")).forEach((cell, colIdx) => {
        ...
        if (!cell.dataset.editLabel) {
          ...
          cell.dataset.editLabel = `${termTitle} Row ${rowIdx + 1} - ${headerName}`;
        }
      });
    });
  });
  ...
  if (scale && scale.dataset.protected !== "true") {
    const scaleTitle = scale.querySelector("h3")?.textContent?.trim() || "Scale";
...
function buildEditor() {
  ...
  editableNodes.forEach((node, idx) => {
    ...
    label.textContent = node.dataset.editLabel;
    input.id = id;
...
function bindPhotoUpload() {
  ...
  fileInput.addEventListener("change", () => {
    ...
    reader.onload = () => {
      ...
      setUploadDiagnostics(`Upload status: Student Photo loaded (${file.name}).`, "ok");
      scheduleSecurityArtifactsUpdate();
    };
    reader.onerror = () => {
      setUploadDiagnostics(`Upload status: Failed to read Student Photo (${file.name}).`, "error");
    };
...
function bindLogoUpload() {
  ...
  fileInput.addEventListener("change", () => {
    ...
    reader.onload = () => {
      ...
      setUploadDiagnostics(`Upload status: Institution Logo loaded (${file.name}).`, "ok");
      scheduleSecurityArtifactsUpdate();
    };
    reader.onerror = () => {
      setUploadDiagnostics(`Upload status: Failed to read Institution Logo (${file.name}).`, "error");
    };
    reader.readAsDataURL(file);
  });
}
  ...
  fileInput.addEventListener("change", () => {
    ...
    toWatermarkDataUrl(file)
      .then((watermarkUrl) => {
        ...
        setUploadDiagnostics(`Upload status: Center Watermark loaded (${file.name}).`, "ok");
        scheduleSecurityArtifactsUpdate();
      })
      .catch(() => {
        ...
        fallbackReader.onload = () => {
          ...
          watermark.style.display = "block";
          watermark.style.opacity = "0.44";
          setUploadDiagnostics(`Upload status: Center Watermark loaded (fallback) (${file.name}).`, "ok");
          scheduleSecurityArtifactsUpdate();
        };
        fallbackReader.onerror = () => {
          setUploadDiagnostics(`Upload status: Failed to process Center Watermark (${file.name}).`, "error");
        };
...
function bindTermSeasonControls() {
  ...
  const applyTermTitles = () => {
    ...
    const t1 = order === "fall-spring" ? `Fall ${y1}` : `Spring ${y1}`;
    const t2 = order === "fall-spring" ? `Spring ${y2}` : `Fall ${y2}`;
...
function bindProtectedAuthorizationAndFooter() {
  lockPlainTextNode(
    document.getElementById("university-seal"),
    "University Seal\n(see Institutional Authorization below)"
  );
  lockPlainTextNode(document.getElementById("authorization-title"), "Institutional Authorization");
  lockPlainTextNode(
    document.getElementById("authorization-text"),
    "Authorized upon official review; this document is true and valid."
  );
  lockPlainTextNode(document.getElementById("transcript-end-note"), "For personal use only. This document is true and valid.");
  lockPlainTextNode(document.getElementById("personal-ssn-label"), "Government ID (18-digit)");
}
...
function bindExportPng() {
  ...
  const doExport = async () => {
    if (typeof window.html2canvas !== "function") {
      status.textContent = "Export status: Failed (html2canvas not loaded)";
    ...
    try {
      ...
      status.textContent = "Export status: Rendering PNG...";
      ...
      status.textContent = `Export status: Done (${docId}.png)`;
      ...
      status.textContent = "Export status: Failed to render PNG";
      status.style.color = "#7a0000";
  ...
  btn.addEventListener("click", async () => {
    const ack = document.getElementById("legal-ack");
    if (!ack || !ack.checked) {
      status.textContent = "Export status: Please check the legal acknowledgment in the sidebar before exporting.";
...
setUploadDiagnostics("Upload status: Ready", "neutral");
